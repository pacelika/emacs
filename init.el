...
(defvar config-files (list "set-modes" "multi-cursor" "lsp" "swiper"))

(defun my-load-config-files (files)
  (unless (= (length files) 0)
    (load-file (concat "~/.emacs.d/" (car files) ".el"))
    (my-load-config-files (cdr files))))

(my-load-config-files config-files)
